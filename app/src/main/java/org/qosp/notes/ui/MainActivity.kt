...
class MainActivity : BaseActivity() {
     ...
     * Handles images, audio, and video files, storing video thumbnails as images.
     *
     * @param uri The source URI of the media file to copy
     * @return The new URI in app's private storage, or null if copy failed
     */
    private suspend fun copySharedMedia(uri: Uri): Uri? {
        ...
        val mediaType = when {
            ...
            mimeType.startsWith("video/") -> MediaStorageManager.MediaType.IMAGE // Store video thumbnail as image
            else -> return null
        }
        ...
        val (newUri, _) = activityModel.mediaStorageManager.createMediaFile(mediaType, extension) ?: return null
                ...
                lifecycleScope.launch {
                    ...
                    if (!uris.isNullOrEmpty()) {
                        val attachments = uris.mapNotNull { uri ->
                            val newUri = copySharedMedia(uri)
                            newUri?.let { Attachment.fromUri(this@MainActivity, it) }
                        }.toTypedArray()
